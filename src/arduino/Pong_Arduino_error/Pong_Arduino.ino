...
void CheckAdjustMode(void)
{
  switch (switchPressCount)
  {
    ...
    case SIZE:
      ...
      u8g2.drawStr(screenWidth-u8g2.getStrPixelWidth(strBuf), u8g2.getFontAscent(), strBuf);
...
void loop()
{
  ...
  do {
    if (lostPoint == false)
    {
      ...
      // Draw the borders
      u8g2.drawLine(0, 0, screenWidth-1, 0);
      u8g2.drawLine(0, 0, 0, screenHeight-1);
      u8g2.drawLine(0, screenHeight-1, screenWidth-1, screenHeight-1);

      // Make things look different when in adjust mode vs play mode
      if (inAdjustMode == true)
      {
        u8g2.drawCircle(bx, by, br, U8G_DRAW_ALL); // Draw the ball as a circle
        u8g2.drawFrame(px, py, pWidth, pHeight);   // Draw the paddle as a frame
      }
      else
      {
        u8g2.drawDisc(bx, by, br, U8G_DRAW_ALL);   // Draw the ball as a disc (i.e. solid circle)
        u8g2.drawRBox(px, py, pWidth, pHeight, 2); // Draw paddle as solid box; use rounded corners with a radius of 2
      }

      sprintf(strBuf, "%d", gameScore);
      u8g2.drawStr((px+pWidth+screenWidth-u8g2.getStrPixelWidth(strBuf))/2, screenHeight/2, strBuf);
    }
    else
    {
      // Display the "You Lose" screen
      u8g2.setFont(u8g_font_gdb20r); // switch to bigger font
      strcpy (strBuf, "TRY");
      u8g2.drawStr((screenWidth-u8g2.getStrPixelWidth(strBuf))/2, screenHeight/2, strBuf);
      strcpy (strBuf, "AGAIN!");
      u8g2.drawStr((screenWidth-u8g2.getStrPixelWidth(strBuf))/2, screenHeight, strBuf);
      u8g2.setFont(u8g_font_profont10r); // switch back to our normal font
     }
  } while (u8g2.nextPage());
